...
@testset "SURFStar - Continuous Features" begin
    ...
    for idx1 = 1:size(data, 2) - 1
        for idx2 = idx1+1:size(data, 2)
            ...
            weights = SURFStar.surfstar(data, target, f_type="continuous")
...
@testset "SURFStar - Discrete Features" begin
    ...
    for idx1 = 1:size(data, 2) - 1
        for idx2 = idx1+1:size(data, 2)
            target = Int64.(data[:, idx1] .> data[:, idx2])
            weights = SURFStar.surfstar(data, target, f_type="discrete")
            @test all(weights[idx1] .>= weights[(1:end .!= idx1) .& (1:end .!= idx2)])
            @test all(weights[idx2] .>= weights[(1:end .!= idx1) .& (1:end .!= idx2)])
        end
    end
...
@testset "SURFStar - Exceptions" begin
    ...
    @test_throws DomainError SURFStar.surfstar(data, target, f_type="something_else")
end
